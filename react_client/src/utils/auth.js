...
export function getAuthClient(config = {}) {
	...
	// Technique called currying which allows us to have a set of functions that all share the same configuration,
	// without having to pass that configuration to each function individually.
	// Somewhat akin to dependency injection
	return {
		debug,
		login,
		logout,
...
// The above code is a lightweight library for managing OAuth tokens.
// You could also use an existing library; many HTTP request libraries like Axios
// or request have middleware to assist with OAuth requests already.
// We wanted to create our own to help better explain how this all works.
